...
/**
 * SpringNestedScrollView is just like {@link android.widget.ScrollView}, but it supports acting
 * as both a nested scrolling parent and child on both new and old versions of Android.
 * Nested scrolling is enabled by default.
 */
public class SpringNestedScrollView extends FrameLayout implements NestedScrollingParent3,
        NestedScrollingChild3, ScrollingView {
    static final int ANIMATED_SCROLL_GAP = 250;

    static final float MAX_SCROLL_FACTOR = 0.5f;

    private static final String TAG = "SpringNestedScrollView";
    ...
    public interface OnScrollChangeListener {
         ...
         */
        void onScrollChange(SpringNestedScrollView v, int scrollX, int scrollY,
    ...
    @VisibleForTesting
    @NonNull
    public SpringEdgeEffect mEdgeGlowTop;

    /** @hide */
    @RestrictTo(LIBRARY)
    @VisibleForTesting
    @NonNull
    public SpringEdgeEffect mEdgeGlowBottom;
    ...
    public SpringNestedScrollView(@NonNull Context context, @Nullable AttributeSet attrs,
        ...
        mEdgeGlowTop = new SpringEdgeEffect(this, RecyclerView.EdgeEffectFactory.DIRECTION_TOP);
        mEdgeGlowBottom = new SpringEdgeEffect(this, RecyclerView.EdgeEffectFactory.DIRECTION_BOTTOM);
    ...
    protected float getBottomFadingEdgeStrength() {
        ...
        final SpringNestedScrollView.LayoutParams lp = (LayoutParams) child.getLayoutParams();
        final int length = getVerticalFadingEdgeLength();
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsBeingDragged) {
                    ...
                    if (canOverscroll) {
                        ...
                        if (pulledToY < 0) {
                            mEdgeGlowTop.handlePullDistance((float) -deltaY / getHeight(),
                                    ev.getX(activePointerIndex) / getWidth());
                            if (!mEdgeGlowBottom.isFinished()) {
                                mEdgeGlowBottom.onRelease();
                            }
                        } else if (pulledToY > range) {
                            mEdgeGlowBottom.handlePullDistance((float) deltaY / getHeight(),
    ...
    private boolean edgeEffectFling(int velocityY) {
        ...
        if (mEdgeGlowTop.getPullDistance() != 0) {
            mEdgeGlowTop.onAbsorb(velocityY);
        } else if (mEdgeGlowBottom.getPullDistance() != 0) {
    ...
    private boolean stopGlowAnimations(MotionEvent e) {
        ...
        if (mEdgeGlowTop.getPullDistance() != 0) {
            mEdgeGlowTop.handlePullDistance(0, e.getY() / getHeight());
            stopped = true;
        }
        if (mEdgeGlowBottom.getPullDistance() != 0) {
            mEdgeGlowBottom.handlePullDistance(0, 1 - e.getY() / getHeight());
            stopped = true;
        }
        return stopped;
    }
    ...
    int getScrollRange() {
        ...
        if (getChildCount() > 0) {
            View child = getChildAt(0);
            SpringNestedScrollView.LayoutParams lp = (LayoutParams) child.getLayoutParams();
            int childSize = child.getHeight() + lp.topMargin + lp.bottomMargin;
            int parentSpace = getHeight() - getPaddingTop() - getPaddingBottom();
    ...
    public boolean pageScroll(int direction) {
        ...
        if (down) {
            ...
            if (count > 0) {
                ...
                SpringNestedScrollView.LayoutParams lp = (LayoutParams) view.getLayoutParams();
    ...
    private int releaseVerticalGlow(int deltaY, float x) {
        ...
        if (mEdgeGlowTop.getPullDistance() != 0) {
            consumed = -mEdgeGlowTop.handlePullDistance(-pullDistance, displacement);
            if (mEdgeGlowTop.getPullDistance() == 0) {
                mEdgeGlowTop.onRelease();
            }
        } else if (mEdgeGlowBottom.getPullDistance() != 0) {
            consumed = mEdgeGlowBottom.handlePullDistance(pullDistance,
                    1 - displacement);
            if (mEdgeGlowBottom.getPullDistance() == 0) {
                mEdgeGlowBottom.onRelease();
            }
    ...
    static class AccessibilityDelegate extends AccessibilityDelegateCompat {
        ...
        public boolean performAccessibilityAction(View host, int action, Bundle arguments) {
            ...
            final SpringNestedScrollView nsvHost = (SpringNestedScrollView) host;
